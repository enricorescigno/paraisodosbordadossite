...
export class ProductAdapter {
  static toLegacyProduct(product: Product): LegacyProduct {
    ...
    const getImageUrl = () => {
      ...
      if (product.images) {
        if (typeof product.images === 'object' && 'primary' in product.images && product.images.primary) {
          return typeof product.images.primary === 'string' ? product.images.primary : product.images.primary.url || product.images.primary.toString();
        }
        if (Array.isArray(product.images) && product.images.length > 0) {
          return product.images[0];
        }
        if (typeof product.images === 'object' && !Array.isArray(product.images) && !('primary' in product.images)) {
    ...
    const getPriceValue = () => {
      ...
      if (typeof product.price === 'object' && product.price && 'value' in product.price) return product.price.value;
      return 'Sob consulta';
    };

    // Helper to get original price safely
    const getOriginalPrice = () => {
      if (typeof product.price === 'object' && product.price && 'originalValue' in product.price) return product.price.originalValue;
      return product.originalPrice;
    };

    // Helper to get discount safely
    const getDiscount = () => {
      if (typeof product.price === 'object' && product.price && 'discount' in product.price) return product.price.discount;
    ...
    const getRatingValue = () => {
      ...
      if (typeof product.rating === 'object' && product.rating && 'value' in product.rating) return product.rating.value;
      return undefined;
    };
    ...
    const getFeatures = () => {
      ...
      if (typeof product.features === 'object' && product.features && 'specifications' in product.features) {
    ...
    const getImagesArray = () => {
      ...
      if (typeof product.images === 'object' && product.images && 'gallery' in product.images && Array.isArray(product.images.gallery)) {
        return product.images.gallery.map(img => typeof img === 'string' ? img : img.url || img.toString());
      }
      if (typeof product.images === 'object' && product.images && !Array.isArray(product.images) && !('primary' in product.images)) {
