...
const AllProductsPage = () => {
  ...
  useEffect(() => {
    ...
    // Apply search filter
    if (searchQuery) {
      result = result.filter(product => product.name.toLowerCase().includes(searchQuery.toLowerCase()) || product.category.toLowerCase().includes(searchQuery.toLowerCase()));
    }
      ...
      <section className={`py-10 md:py-16 bg-[#f5f5f7] ${isMobile ? 'pt-24' : 'pt-20'}`}>
        <div className="container-custom">
          <div className="mb-8 space-y-4">
            <h1 className="text-3xl md:text-4xl font-bold text-brand-red text-center">Bem-Vindo à Paraíso dos Bordados</h1>
            ...
            <div className="relative max-w-md mx-auto">
              <input type="text" placeholder="Buscar produtos..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-full border border-gray-300 
          ...
          <Tabs defaultValue="all" className="mb-8 justify-center flex flex-col items-center">
            <TabsList className="bg-white rounded-full shadow-sm overflow-x-auto py-1 px-1 w-auto flex flex-nowrap">
              {categories.map(category => <TabsTrigger key={category} value={category} onClick={() => setActiveCategory(category)} className="px-4 py-2 rounded-full data-[state=active]:bg-brand-red data-[state=active]:text-white">
          ...
          {loading ? <div className="flex justify-center items-center py-20">
              ...
              <div className="hidden md:grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                {filteredProducts.map(product => <ProductCard key={product.id} product={product} whatsappNumber={whatsappNumber} />)}
              </div>
              ...
              <div className="md:hidden">
                <Carousel className="w-full">
                  ...
                  <div className="flex justify-center gap-2 mt-6">
                    {Array.from({
                  length: Math.min(5, Math.ceil(filteredProducts.length / 2))
                }).map((_, index) => <div key={index} className={`h-2 w-2 rounded-full ${index === 0 ? 'bg-brand-red' : 'bg-gray-300'}`}></div>)}
                  </div>
                </Carousel>
              </div>
...
const ProductCard = ({
  ...
  return <Card className="rounded-xl overflow-hidden border-0 bg-white shadow-sm transition-all duration-300 
        hover:shadow-md hover:-translate-y-1 hover:scale-[1.02] group">
      <Link to={`/produto/${product.id}`} className="block">
        <AspectRatio ratio={1 / 1} className="relative bg-[#f8f8f8]">
          <img src={product.imageUrl || "https://via.placeholder.com/300x300?text=Sem+Imagem"} alt={product.name} className="w-full h-full object-contain mix-blend-multiply p-4" />
          {product.isNew && <div className="absolute top-3 right-3">
      ...
      <CardContent className="p-4">
        ...
        {product.colors && <div className="flex gap-1.5 mb-3">
          ...
          return <div key={index} className="w-4 h-4 rounded-full border border-gray-200 cursor-pointer" style={{
        ...
        <div className="flex items-center justify-between">
          <a href={`https://wa.me/${whatsappNumber}?text=${generateWhatsAppMessage(product.name)}`} target="_blank" rel="noopener noreferrer" className="text-xs md:text-sm text-gray-600 hover:text-brand-red flex items-center gap-1 transition-all group-hover:translate-x-1">
            <MessageCircle className="h-3.5 w-3.5 md:h-4 md:w-4" />
            <span>Solicitar orçamento</span>
          </a>
          <Link to={`/produto/${product.id}`} className="text-brand-dark hover:text-brand-red transition-all group-hover:translate-x-1">
