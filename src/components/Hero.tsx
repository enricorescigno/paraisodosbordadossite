...
const Hero = () => {
  ...
  return <section className="relative w-full h-screen overflow-hidden">
      ...
      <video ref={videoRef} autoPlay muted loop playsInline className={`absolute inset-0 w-full h-full object-cover scale-[1.01] transition-opacity duration-1000 ${isVideoLoaded ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="relative z-20 flex items-center justify-center h-full">
        <div className="container-custom text-center">
          ...
          <p className="text-white/90 text-lg md:text-xl max-w-2xl mx-auto mb-8 animate-fade-in opacity-0" style={{
          animationDelay: '0.7s',
          animationFillMode: 'forwards'
        }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 animate-fade-in opacity-0" style={{
          animationDelay: '0.9s',
          animationFillMode: 'forwards'
        }}>
            ...
            <Link to="/portfolio" className="btn-secondary px-6 py-3">Ver Portfólio de Bordados</Link>
          </div>
        </div>
      </div>
