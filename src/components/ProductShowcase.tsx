      ...
      <div className="container-custom">
        ...
        <div className={`flex justify-center mb-10 overflow-x-auto pb-2 transform transition-all duration-700 delay-100 ${sectionInView ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="flex gap-2 md:gap-3 bg-white/80 backdrop-blur-sm p-1 rounded-full shadow-sm">
            {categories.map(category => <button key={category} onClick={() => setActiveTab(category)} className={`px-4 py-2 whitespace-nowrap rounded-full transition-all duration-300
                  ${activeTab === category ? 'bg-brand-red text-white shadow-md' : 'bg-transparent text-brand-dark hover:bg-brand-red/10'}`}>
        ...
        <div className="hidden md:block">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 relative">
            {visibleProducts.map((product, index) => <ProductCard key={product.id} product={product} index={index} animateProducts={animateProducts} whatsappNumber={whatsappNumber} />)}
          </div>
        </div>
        ...
        <div className="md:hidden">
          <Carousel className="w-full">
            <CarouselContent>
              {visibleProducts.map((product, index) => <CarouselItem key={product.id} className="basis-full sm:basis-1/2">
                  <ProductCard product={product} index={index} animateProducts={animateProducts} whatsappNumber={whatsappNumber} />
                </CarouselItem>)}
            </CarouselContent>
            <div className="flex justify-center gap-2 mt-6">
              {Array.from({
              length: Math.min(5, Math.ceil(visibleProducts.length / 2))
...
const ProductCard = ({
  ...
  return <Card className={`rounded-xl overflow-hidden border-0 bg-white shadow-sm transition-all duration-500 
        ${animateProducts ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-8 opacity-0 scale-95'}
        hover:shadow-md hover:-translate-y-1 hover:scale-[1.02] group`} style={{
    transitionDelay: `${index * 100}ms`
  }}>
      <Link to={`/produto/${product.id}`} className="block">
        <AspectRatio ratio={1 / 1} className="relative bg-[#f8f8f8]">
          <img src={product.image} alt={product.name} className="w-full h-full object-contain mix-blend-multiply p-4" onError={e => {
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between">
          <a href={`https://wa.me/${whatsappNumber}?text=${generateWhatsAppMessage(product.name)}`} target="_blank" rel="noopener noreferrer" className="text-xs md:text-sm text-gray-600 hover:text-brand-red flex items-center gap-1 transition-all duration-300 group-hover:translate-x-1">
            <MessageCircle className="h-3.5 w-3.5 md:h-4 md:w-4" />
            <span>Solicitar orçamento</span>
          </a>
          <Link to={`/produto/${product.id}`} className="text-brand-dark hover:text-brand-red transition-all duration-300 group-hover:translate-x-1">
