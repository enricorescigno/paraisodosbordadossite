  ...
  return <motion.div initial={{
    ...
    transition: {
      ...
      <div className="w-full aspect-square bg-white rounded-2xl p-6 mb-4 overflow-hidden relative shadow-sm hover:shadow-md transition-shadow duration-300">
        <motion.img src={optimizedImageUrl} alt={`Produto: ${name}`} className="w-full h-full object-contain mix-blend-multiply" loading="lazy" whileHover={{
        scale: 1.05
      }} transition={{
        duration: 0.3
      }} onError={e => {
        ...
        {(isNew || Number(id) === 204) && <div className="absolute top-3 right-3">
      ...
      <div className="flex flex-col flex-grow w-full">
        ...
        {description && <p className="text-center text-gray-500 mb-4 text-sm line-clamp-2 h-[2.5rem] overflow-hidden mx-auto max-w-[90%] md:text-sm">
            {description}
          </p>}
      </div>
      
      {/* Renderizar o botão apenas se showActionButton for true */}
      {showActionButton && <div className="flex justify-center mt-auto w-full">
          <Link to={`/produto/${id}`} className="w-full sm:w-auto flex justify-center">
            <Button variant="default" size="lg" className="rounded-full px-8 w-full max-w-[200px] min-h-[48px]" aria-label={`Ver detalhes de ${name}`}>
